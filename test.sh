...
cd Sn1per/;
sh install.sh;
expect "[This script will install sniper under /usr/share/sniper. Are you sure you want to continue? (Hit Ctrl+C to exit)" {send "\r"};
cd ~/tools;
git clone https://github.com/1N3/IntruderPayloads.git;
cd IntruderPayloads/;
./install.sh;
cd ~/tools;
sudo apt install keepnote -y;
#wget https://github.com/Arachni/arachni/releases/download/v1.5.1/arachni-1.5.1-0.5.12-linux-x86_64.tar.gz;
#tar -zxvf arachni-1.5.1-0.5.12-linux-x86_64.tar.gz;
#cd arachni-1.5.1-0.5.12;
sudo apt install arachni -y;
cd /opt;
